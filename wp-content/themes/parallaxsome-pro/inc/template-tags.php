...
function parallaxsome_posted_on() {
	...
	$posted_on = sprintf(
		esc_html( '%s', __('post date', 'parallaxsome-pro') ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html( '%s', __('post author', 'parallaxsome-pro' )),
	...
	if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'parallaxsome-pro' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
...
function parallaxsome_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'parallaxsome-pro' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'parallaxsome-pro' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'parallaxsome-pro' ),
