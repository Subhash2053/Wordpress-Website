<?php
	...
	if($show_success_notice) :
		...
		<div class="action-tab success">
			<h3><?php printf( 'All Recommended action has been successfully completed.', 'parallaxsome-pro'); ?></h3>
			<a class="button button-primary" href="<?php echo esc_url(admin_url('customize.php')); ?>"><?php echo esc_html__('Customize Theme', 'parallaxsome-pro'); ?></a>
		</div>
		<?php
	else :
		...
		foreach($req_plugins as $plugin) :
			if($plugin['bundled'] == false) {
				...
				if(!isset($info->errors)) :
					...
					switch($status) {
						case 'install' :
							...
							$label = esc_html__('Install and Activate', 'parallaxsome-pro');
							break;

						case 'inactive' :
							$btn_class = 'button';
							$label = esc_html__('Deactivate', 'parallaxsome-pro');
							break;

						case 'active' :
							$btn_class = 'activate button button-primary';
							$label = esc_html__('Activate', 'parallaxsome-pro');
							break;
					}
					...
					<div class="action-tab warning">
						<h3><?php printf( esc_html__("Install : %s Plugin", 'parallaxsome-pro'), $info->name ); ?></h3>
						<p><?php esc_html_e('Install Contact Form 7 to add the contact forms.', 'parallaxsome-pro'); ?></p>
				...
				?>
				<div class="action-tab warning">
					<h3><?php printf( esc_html__("Install : %s Plugin", 'parallaxsome-pro'), $plugin['name'] ); ?></h3>
					<p><?php esc_html_e('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'parallaxsome-pro'); ?></p>
