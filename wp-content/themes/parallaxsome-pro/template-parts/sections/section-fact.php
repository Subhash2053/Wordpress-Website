...
<?php
	...
	if( $section_option != 'hide' ) {
		$section_title = get_theme_mod( 'fact_section_title', esc_html__( 'Fact About Us', 'parallaxsome-pro' ) );
		$section_sub_title = get_theme_mod( 'fact_section_sub_title', esc_html__( 'Our Works', 'parallaxsome-pro' ) );
		$section_description = get_theme_mod( 'fact_section_description', '' );
		$section_bg_image = get_theme_mod( 'fact_bg_image', '' );
        $fact_section_layout = get_theme_mod('fact_section_layout');
?>
		<section class="ps-home-section <?php echo esc_attr($fact_section_layout); ?>" id="section-fact" <?php if($section_bg_image){?> style="background-image: url(<?php echo esc_url($section_bg_image); ?>);"<?php } ?>>
			<?php if($fact_section_layout != 'layout-2'){ ?><div class="ps-section-container"><?php } ?>
				<div class="fact-title wow fadeInUp" data-wow-duration="0.5s">
			...
			<?php if($fact_section_layout != 'layout-2'){ ?></div><?php } ?><!-- .ps-section-container -->
		</section>
<?php } ?>