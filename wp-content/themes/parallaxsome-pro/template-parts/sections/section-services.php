...
<?php
	...
	if( $section_option != 'hide' ) {
		$section_title = get_theme_mod( 'services_section_title', esc_html__( 'Our Services', 'parallaxsome-pro' ) );
		$section_sub_title = get_theme_mod( 'services_section_sub_title', esc_html__( 'Our Works', 'parallaxsome-pro' ) );
		$section_description = get_theme_mod( 'services_section_description', '' );
		$section_bg_image = get_theme_mod( 'service_bg_image', '' );
?>
		<section class="ps-home-section" id="section-services" <?php if($section_bg_image){?> style="background-image: url(<?php echo esc_url($section_bg_image); ?>);"<?php } ?>>
			<div class="ps-section-container clearfix">
				<div class="service-title wow fadeInUp" data-wow-duration="0.5s">
											...
											<div class="tab-pane" id="stab-<?php echo esc_attr( $i ).'-'.esc_attr( $service_page_id_value ); ?>">
												...
												<div <?php if( has_post_thumbnail() ) { ?>class="content-right" <?php } ?>><br>
													...
													<a class="button ps-btn" href="<?php the_permalink(); ?>" target="_blank">
														<?php esc_html_e('Learn More','parallaxsome-pro'); ?>
													</a>
												</div>
											</div>
						<?php
